...
@section('title','Today Checker Info')
@section('content')
<div class="content-i">
            ...
            <div class="col-lg-12">
                ...
                <div class="element-wrapper">
                    ...
                    <div class="element-box">
                        <form action="{{route('todaycheck.store')}}" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
                        @csrf
                            <h5 class="form-desc">
								আজকের গাড়ীর চেকের  ইনফরমেশন যোগ  করুণ
                            </h5>
                            <div class="form-group row">
                                <label class="col-form-label col-sm-2 offset-md-2" for="">চেকারের নাম* </label>
                                <div class="col-sm-5">
									<select name="checker_id" id="checker_id" class="form-control">
										@foreach($checker as $checkers)
										<option value="{{$checkers->id}}">{{$checkers->name}}</option>
										@endforeach
									</select>
                                </div>
							</div>
							<div class="form-group row">
								...
								<div class="col-sm-5">
                                	<select name="checkpost_id" id="checkpost_id" class="form-control ">
										...
										<option value="{{$chpost->id}}">{{$chpost->name}}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group row">
								...
								<div class="col-sm-5">
									<input type="date" class="form-control" name="check_date" value="{{old('check_date')}}">
								</div>
							</div>
							<div class="form-group row">
								<label class="col-form-label col-sm-2 offset-md-2" for="">চেকের সময় </label>
								<div class="col-sm-5">
									<input type="time" class="form-control" name="check_time" value="{{old('check_time')}}">
								</div>
							</div>
