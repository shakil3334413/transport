                            ...
                            @foreach($allbuscost as $key=>$eachbus)
								<tr>
									...
									<td class="text-center">
										<a title="এডিট" class="btn btn-primary btn-sm" href="{{route('allbuscost.edit',$eachbus->id)}}"><i class="fa fa-edit"></i></a>
										...
										<form id="delete-form-{{ $eachbus->id }}" action="{{ route('allbuscost.destroy',$eachbus->id) }}"  style="display: none;" method="POST">
											@csrf
											@method('DELETE')
										</form>
									</td>
